...
"${P[@]}" -f tests/stubs.sql 2>/dev/null; "${P[@]}" -f schema.sql; "${P[@]}" -f 002_photos_and_realtime.sql 2>/dev/null; "${P[@]}" -f 003_interactions.sql; "${P[@]}" -f 004_memories.sql; "${P[@]}" -f 005_drawings_trips_games_calendar.sql

...
/**
 * 🎲 surprise us. It rolls from what you both like (or what either of you said "saturday?" to), leans toward
 * recent finds, and never shows the same thing twice in a row. Only if nothing is shared yet does it roll from
 * everything open, and it says so.
 */
export function PickForUs() {
  ...
  const [fromShared, setFromShared] = useState(true);
          ...
          {phase === "result" && pick ? (
            <motion.div key={pick.id} initial={{ opacity: 0, y: 16, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8 }} transition={{ type: "spring", stiffness: 360, damping: 24 }}>
              <div className="rounded-[24px] bg-white/80 p-4">
                ...
                {!fromShared && <p className="mt-1.5 text-[13px] text-ink/55">nothing you both like yet, so this one&apos;s just a fun find.</p>}
              </div>
              <div className="mt-3 flex gap-2">
                <PillButton onClick={accept} className="flex-1">let&apos;s do it</PillButton>
                <PillButton tone="glass" onClick={spin} className="flex-1">not today</PillButton>
              </div>
            </motion.div>
          ) : (
